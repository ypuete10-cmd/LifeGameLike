...
const GENE_INFO := {
	"photosyn": {
		"name": "光合成セル
		(photosynthetic cell)",
		"desc": "生存するたびにスコア+2を得る。
		(Gain +2 points each time you survive)",
		"cost": 2,
	},
	"guardian": {
		"name": "守護セル
		(Guardian Cell)",
		"desc": "死亡時に隣接セル1つを救う。スコア+1。
		(Upon death, saves one adjacent cell. Score +1)",
		"cost": 3,
	},
	"explode": {
		"name": "爆発セル
		(Explosive cell)",
		"desc": "死亡時に周囲を巻き込んで倒す。スコア+3。
		(Upon death, takes down those nearby. Score +3)",
		"cost": 4,
	},
	"copy": {
		"name": "コピーセル
		(Copy cell)",
		"desc": "誕生時に隣接セル1つを自分と同じにする。スコア+1。
		(Upon spawning, change one adjacent cell to match its own state. Score +1
...
const ITEM_INFO := {
	"delete_token": {
		"name": "削除トークン
		(Delete token)",
		"desc": "遺伝子を1つ削除できるアイテム。
		(An item that can remove one gene.)",
		"cost": 2,
		"type": "item",
	},
}
